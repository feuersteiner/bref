...
const generateSoftColor = (baseColor: string, mode: ThemeMode): string =>
	mode === 'light'
		...
		: colord(baseColor).darken(0.25).desaturate(0.2).toHex();
...
/**
 * Generates a minimal set of color tokens based on a base color and theme mode.
 * Produces only soft, base, and contrast variants. Hover/active states are
 ...
 * @param mode - The current theme mode ('light' or 'dark')
 * @returns An object containing soft, base, and contrast color tokens
 */
...
const setThemeMode = (userTheme: ThemeProps, mode: ThemeMode) => {
	...
	Object.entries(theme.surfaceHex).forEach(([surface, value]) => {
		setColorTokens(surface, value, mode);
	});
